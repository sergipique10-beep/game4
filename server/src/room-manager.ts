...
import { AIDifficulty, CellValue, ClientId, MoveOutcome, Room } from './types';
...
export class RoomManager {
  ...
  private matchmakingQueue: ClientId[] = [];
  ...
  createAIRoom(playerId: ClientId, difficulty: AIDifficulty = 'easy'): Room {
    const room: Room = {
      code: this.uniqueCode(),
      board: createEmptyBoard(),
      currentPlayer: 1,
  ...
  joinMatchmaking(clientId: ClientId): Room | null {
    if (this.matchmakingQueue.includes(clientId)) return null;
    const opponentId = this.matchmakingQueue.shift();
    if (opponentId) {
      const room = this.createRoom(opponentId);
      this.joinRoom(room.code, clientId);
      return room;
    }
    this.matchmakingQueue.push(clientId);
    return null;
  }

  leaveMatchmaking(clientId: ClientId): void {
    this.matchmakingQueue = this.matchmakingQueue.filter((id) => id !== clientId);
  }
